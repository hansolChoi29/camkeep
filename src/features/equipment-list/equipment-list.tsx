...
export default function EauipmentList(props: EquipmentListProps) {
  const { selected, list, errors } = props;
        ...
        {errors[selected] ? (
          ...
          <ul className="flex flex-wrap  gap-2  justify-center ">
            {list.map((item, i) => (
              <li
                key={i}
                className="border w-40 h-30 flex flex-col items-center  sm:w-80 sm:h-90   border-[#E07B39] rounded-lg overflow-hidden bg-white"
              >
                <Image
                  src={item.image}
